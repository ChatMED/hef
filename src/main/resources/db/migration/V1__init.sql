...
create table if not exists hef_users
(
    id                  bigint generated by default as identity
        primary key,
...
create unique index if not exists answers_question_id_model_id on answers (question_id, model_id);
create unique index if not exists evaluations_answer_id_user_id on evaluations (answer_id, user_id);
create index if not exists evaluations_question_id_user_id on evaluations (question_id, user_id);
create unique index if not exists models_name on models (name);
create unique index if not exists questions_question_key on questions (question_key);
create unique index if not exists hef_users_username on hef_users (username);